...
class Wpfunos_ServiciosV2 {
  ...
  public function __construct( $plugin_name, $version ) {
    ...
    add_action('wp_ajax_nopriv_wpfunos_ajax_serviciosv2_mail_detalles', function () { $this->wpfunosServiciosMailDetalles();});
    add_action('wp_ajax_wpfunos_ajax_serviciosv2_mail_detalles', function () {$this->wpfunosServiciosMailDetalles();});
  ...
  */
  ...
  public function wpfunosServiciosDatosShortcode($atts, $content = ""){
    ...
    if(!isset($_GET['lat'])){
      ?><script id="wpfunos-serviciosv2-home"> $("#wpfunos-servicios-geo").hide(); console.log('unwanted'); console.log(new Date()); window.location.href = "/"; </script><?php
      return;
    }
    ...
    if( $_GET['wpfref'] == 'dummy' || !isset($_COOKIE['wpfe']) || !isset($_COOKIE['wpft']) || !isset($_COOKIE['wpfn']) ){
      ...
      console.log('ya está registrado. Creando entrada.');
      console.log(new Date());
  ...
  */
  public function wpfunosServiciosBotonLlamame(){
    ...
    if ( !wp_verify_nonce( $_POST['wpnonce'], "wpfunos_serviciosv2_nonce".apply_filters('wpfunos_userIP','dummy'))) {
  ...
  */
  ...
  public function wpfunosServiciosMailDetalles(){
    do_action('wpfunos_log', '==============' );
    do_action('wpfunos_log', 'Llegada ajax Servicio BotonDetalles' );
    //if ( !wp_verify_nonce( $_POST['wpnonce'], "wpfunos_serviciosv2_nonce")) {
    if ( !wp_verify_nonce( $_POST['wpnonce'], "wpfunos_serviciosv2_nonce".apply_filters('wpfunos_userIP','dummy'))) {
    ...
    if( !apply_filters('wpfunos_reserved_email','dummy') ){
      $my_post = array(
        ...
        'meta_input'   => array(
          ...
          'wpfunos_userServicioEnviado' => true,
          'wpfunos_userAceptaPolitica' => true,
          'wpfunos_userAccion' => '9',
          'wpfunos_userNombreAccion' => 'Usuario pide correo detalles',
          'wpfunos_userServicioTitulo' => sanitize_text_field( get_the_title( $_POST['servicio'] ) ),
          'wpfunos_userServicioEmpresa' => sanitize_text_field( get_post_meta( $_POST['servicio'], 'wpfunos_servicioEmpresa', true ) ),
          'wpfunos_userServicioPoblacion' => sanitize_text_field( get_post_meta( $_POST['servicio'], 'wpfunos_servicioPoblacion', true ) ),
          'wpfunos_userServicioProvincia' => sanitize_text_field( get_post_meta( $_POST['servicio'], 'wpfunos_servicioProvincia', true ) ),
    ...
    if( get_option('wpfunos_activarCorreoUsuarioDetalles') && !apply_filters('wpfunos_reserved_email','dummy')){
      $headers[] = 'Content-Type: text/html; charset=UTF-8';
      ...
      $mensaje = str_replace( '[comentarios]' , get_post_meta( $_POST['servicio'], 'wpfunos_servicio'.$destino.$ataud.$velatorio.$despedida.'_Comentario', true) , $mensaje );
      $mensaje = str_replace( '[IP]' , apply_filters('wpfunos_userIP','dummy') , $mensaje );
      $mensaje = str_replace( '[logoServicio]' , wp_get_attachment_image ( get_post_meta( $_POST['servicio'], 'wpfunos_servicioLogo', true ) ,'full' ) , $mensaje );
      $mensaje = str_replace( '[imagenconfirmado]' , wp_get_attachment_image (  get_post_meta( get_option('wpfunos_postConfImagenes') , 'wpfunos_imagenConfirmado', true ) , array(45,46)) , $mensaje );
      $mensaje = str_replace( '[imagenecologico]' , wp_get_attachment_image (  get_post_meta( get_option('wpfunos_postConfImagenes') , 'wpfunos_imagenEcologico', true ) , array(60,60)) , $mensaje );
      $mensaje = str_replace( '[imagenpromo]' , wp_get_attachment_image ( get_post_meta( $_POST['servicio'], 'wpfunos_servicioImagenPromo', true ) ,'full' ) , $mensaje );
      $mensaje = str_replace( '[nombrepack]' , get_post_meta( $_POST['servicio'], 'wpfunos_servicioPackNombre', true ) , $mensaje );
      $mensaje = str_replace( '[textoconfirmado]' , 'Precio confirmado' , $mensaje );
      $mensaje = str_replace( '[textoprecio]' , get_post_meta( $_POST['servicio'], 'wpfunos_servicioTextoPrecio', true ) , $mensaje );
      ...
      wp_mail (  $_POST['email'] , get_option('wpfunos_asuntoCorreoUsuarioDetalles') , $mensaje, $headers );
      //update_post_meta( $post_id, 'wpfunos_userLead', true );
      do_action('wpfunos_log', '==============' );
      do_action('wpfunos_log', 'Enviado correo detalles ' .$_POST['email'] );
      do_action('wpfunos_log', 'Enviado CCO ' . apply_filters('wpfunos_dumplog', get_option('wpfunos_mailCorreoCcoUsuarioDetalles' ) ) );
      do_action('wpfunos_log', 'Enviado BCC ' . apply_filters('wpfunos_dumplog', get_option('wpfunos_mailCorreoBccUsuarioDetalles' ) ) );
      do_action('wpfunos_log', 'userIP: ' . apply_filters('wpfunos_userIP','dummy') );
      do_action('wpfunos_log', 'Nombre: ' . $_POST['nombre'] );
      do_action('wpfunos_log', '$referencia: ' . $referencia );
    }
