...
class Wpfunos_ServiciosV3
{
  ...
  public function wpfunosV3UltimaBusquedaShortcode($atts, $content = "")
  {
    ...
    if (isset($_COOKIE['wpfu'])) {
      ...
      //if( strlen( $_COOKIE['wpfu'] ) > 1 ) do_action('wpfunos_log', $userIP.' - 0500 '.'wpfu: ' .apply_filters('wpfunos_dumplog', $wpfu ) );
...
?>
  ...
  public function wpfunosV3ResultadosShortcode($atts, $content = "")
  {
    ...
    ?>
      <script>
        console.log('Cargando popups Elementor.');
      </script>
      <?php
      ...
      ?>
      <script>
        console.log('Cargando popups Elementor END.');
      </script>
      <?php
  ...
  public function wpfunosV3ColumnaDerechaShortcode($atts, $content = "")
  {
    ...
    ?>
      <h6 style="text-align: center;margin-top: 20px;border-style: solid;border-width: thin;padding: 20px;border-radius: 4px;"><?php echo $texto; ?> <?php echo $address; ?></h6>
    <?php
    }
  ...
  public function wpfunosV3ImagesShortcode($atts, $content = "")
  {
    ...
    switch ($a['imagen']) {
      case 'logo':
        echo $_GET['valor-logo'];
        break;
      case 'confirmado':
        echo $_GET['valor-logo-confirmado'];
        break;
    }
  }
  ...
  public function wpfunosConfirmacionShortcode($atts, $content = "")
  {
    ...
    //if( strlen( $_COOKIE['wpfu'] ) > 1 ) do_action('wpfunos_log', $userIP.' - 0500 '.'wpfunos-confirmacion: ' .apply_filters('wpfunos_dumplog', $_GET ) );
    switch ($a['campo']) {
      case 'Nombre':
      ...
      case 'email':
        ...
        break;
      case 'telefono':
        return $_GET['usuario_telefono'];
        break;
      case 'ubicacion':
        return $_GET['ubicacion'];
        break;
      case 'cuando':
        return $_GET['cuando'];
        break;
      ...
      case 'destino':
        if ($_GET['cf']['resp1'] == '1') return 'Entierro';
        if ($_GET['cf']['resp1'] == '2') return 'Incineración';;
        break;
      case 'ataud':
        if ($_GET['cf']['resp2'] == '1') return 'Ataúd medio';
        if ($_GET['cf']['resp2'] == '2') return 'Ataúd económico';
        if ($_GET['cf']['resp2'] == '3') return 'Ataúd premium';
        break;
      case 'velatorio':
        if ($_GET['cf']['resp3'] == '1') return 'Velatorio';
        if ($_GET['cf']['resp3'] == '2') return 'Sin velatorio';
        break;
      case 'ceremonia':
        if ($_GET['cf']['resp4'] == '1') return 'Sin ceremonia';
        if ($_GET['cf']['resp4'] == '2') return 'Solo sala';
        if ($_GET['cf']['resp4'] == '3') return 'Ceremonia civil';
        if ($_GET['cf']['resp4'] == '4') return 'Ceremonia religiosa';
        break;

      case 'Referencia':
        return $_GET['wpfnewref'];
        break;
...
//    do_action('wpfunos_log','results: ' . apply_filters('wpfunos_dumplog', $results));
  ...
  public function wpfunosResultV3ConfirmadoDummy($wpfunos_confirmado)
  {
    if (is_array($wpfunos_confirmado) && count($wpfunos_confirmado) != 0) {
      $IP = apply_filters('wpfunos_userIP', 'dummy');
    ?>
      <div class="wpfunos-titulo" id="wpfunos-titulo-confirmado">
        <p></p>
        <center>
          <h2>Precio confirmado</h2>
        </center>
      </div>
      <?php
      ...
      foreach ($wpfunos_confirmado as $value) {
        ...
        if (
          $transient_id === false || $transient_id['wpfadr'] != $address || $transient_id['wpfdist'] != $_GET['distance'] || $transient_id['wpflat'] != $_GET['lat'] || $transient_id['wpflng'] != $_GET['lng']
      ...
      ?>
        <div class="wpfunos-busqueda-contenedor" id="wpfunos-busqueda-resultado-
        <?php
        echo $value[0];
        ?>
         ">
    ...
    if (is_array($wpfunos_confirmado) && count($wpfunos_confirmado) != 0) {
      ...
      <div class="wpfunos-titulo" id="wpfunos-titulo-confirmado">
        ...
        <center>
          <h2>
            <?php
            esc_html_e('Precio confirmado', 'wpfunos_es')
            ?></h2>
        </center>
      </div>
      <?php
  ...
  public function wpfunosResultV3SinConfirmar($wpfunos_sinconfirmar)
  {
    if (is_array($wpfunos_sinconfirmar) && count($wpfunos_sinconfirmar) != 0) {
      $IP = apply_filters('wpfunos_userIP', 'dummy');
      ?>
      <div class="wpfunos-titulo" id="wpfunos-titulo-sin-confirmar">
        <p></p>
        <center>
          <h2><?php esc_html_e('Precio sin confirmar', 'wpfunos_es') ?></h2>
        </center>
      </div>
      <?php
...
<?php
  ...
  public function wpfunosFiltros()
  {
    ...
    switch ($_GET['cf']['resp1']) {
      case '1':
        $result['resp1'] = array('desc' => esc_html__('Destino', 'wpfunos_es'), 'inicial' => 'E', 'texto' => esc_html__('Entierro', 'wpfunos_es'));
        break;
      case '2':
        $result['resp1'] = array('desc' => esc_html__('Destino', 'wpfunos_es'), 'inicial' => 'I', 'texto' => esc_html__('Incineración', 'wpfunos_es'));
        break;
    }
    switch ($_GET['cf']['resp2']) {
      case '1':
        $result['resp2'] = array('desc' =>  esc_html__('Ataúd', 'wpfunos_es'), 'inicial' => 'M', 'texto' =>  esc_html__('Ataúd Normal', 'wpfunos_es'));
        break;
      case '2':
        $result['resp2'] = array('desc' =>  esc_html__('Ataúd', 'wpfunos_es'), 'inicial' => 'E', 'texto' =>  esc_html__('Ataúd Económico', 'wpfunos_es'));
        break;
      case '3':
        $result['resp2'] = array('desc' =>  esc_html__('Ataúd', 'wpfunos_es'), 'inicial' => 'P', 'texto' =>  esc_html__('Ataúd Premium', 'wpfunos_es'));
        break;
    }
    switch ($_GET['cf']['resp3']) {
      case '1':
        $result['resp3'] = array('desc' => esc_html__('Velatorio', 'wpfunos_es'), 'inicial' => 'V', 'texto' => esc_html__('Velatorio', 'wpfunos_es'));
        break;
      case '2':
        $result['resp3'] = array('desc' => esc_html__('Velatorio', 'wpfunos_es'), 'inicial' => 'S', 'texto' => esc_html__('Sin velatorio', 'wpfunos_es'));
        break;
    }
    switch ($_GET['cf']['resp4']) {
      case '1':
        $result['resp4'] = array('desc' => esc_html__('Ceremonia', 'wpfunos_es'), 'inicial' => 'S', 'texto' => esc_html__('Sin ceremonia', 'wpfunos_es'));
        break;
      case '2':
        $result['resp4'] = array('desc' => esc_html__('Ceremonia', 'wpfunos_es'), 'inicial' => 'O', 'texto' => esc_html__('Solo sala', 'wpfunos_es'));
        break;
      case '3':
        $result['resp4'] = array('desc' => esc_html__('Ceremonia', 'wpfunos_es'), 'inicial' => 'C', 'texto' => esc_html__('Ceremonia civil', 'wpfunos_es'));
        break;
      case '4':
        $result['resp4'] = array('desc' => esc_html__('Ceremonia', 'wpfunos_es'), 'inicial' => 'R', 'texto' => esc_html__('Ceremonia religiosa', 'wpfunos_es'));
        break;
    }
    return $result;
  }
